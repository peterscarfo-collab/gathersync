...
export default function AdminAttendanceEventScreen() {
  ...
  const { eventId, view } = useLocalSearchParams<{ eventId: string; view?: string | string[] }>();
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    if (selectedView === 'all' || selectedView === 'unchecked' || selectedView === 'attended' || selectedView === 'not-attended') {
  ...
  const isEventInFuture = (current: Event) => {
    if (current.eventType !== 'fixed' || !current.fixedDate) return false;
  ...
  const hasAttendanceOutcomes = (current: Event) => {
    ...
    if (latest.statuses && typeof latest.statuses === 'object') {
      return Object.values(latest.statuses).some((value) => value === 'attended' || value === 'not-attended');
    }
  ...
  const getAttendanceMap = (current: Event): Record<string, AttendanceState> => {
    ...
    const useAttendanceOutcomes = !isEventInFuture(current) && hasAttendanceOutcomes(current);
    ...
    if (latest.statuses && typeof latest.statuses === 'object') {
      participants.forEach((participant) => {
        const value = latest.statuses[participant.id] ?? latest.statuses[participant.name];
    ...
    attended.forEach((name: string) => {
      const participant = participants.find((p) => p.name === name || p.id === name);
  ...
  const persistAttendanceMap = async (current: Event, nextMap: Record<string, AttendanceState>) => {
    ...
    const attendees = participants
      .filter((participant) => (nextMap[participant.id] || 'unchecked') === 'attended')
  ...
  const setAttendanceState = async (participantId: string, nextState: AttendanceState) => {
    if (!event) return;
    ...
    const participant = getActiveParticipants(event).find((entry) => entry.id === participantId);
  ...
  const toggleAttended = async (participantId: string) => {
    ...
    const latestMap = getAttendanceMap(event);
    const currentState = latestMap[participantId] || 'unchecked';
    const nextState: AttendanceState = currentState === 'attended' ? 'unchecked' : 'attended';
  ...
  const toggleNotAttended = async (participantId: string) => {
    ...
    const latestMap = getAttendanceMap(event);
    const currentState = latestMap[participantId] || 'unchecked';
    const nextState: AttendanceState = currentState === 'not-attended' ? 'unchecked' : 'not-attended';
    await setAttendanceState(participantId, nextState);
  };
  ...
  const unarchiveEventOnly = async () => {
    ...
    const runUnarchive = async () => {
      try {
        ...
        if (Platform.OS === 'web') {
          window.alert('Event unarchived successfully. It now appears in the Events tab.');
          return;
        }
  ...
  const visibleParticipants = useMemo(() => {
    ...
    const mapped = event.participants
      ...
      .map((participant) => ({
        participant,
        state: statusMap[participant.id] || statusMap[participant.name] || 'unchecked',
      }));
    ...
    const filtered = normalizedSearch
      ? filteredByStatus.filter((row) => (row.participant.name || '').toLowerCase().includes(normalizedSearch))
      : filteredByStatus;
        ...
        <View style={[styles.card, { backgroundColor: cardBg }]}>
          ...
          <View style={styles.metaSummaryRow}>
            ...
            <Pressable onPress={() => setParticipantView('unchecked')}>
              <ThemedText style={[styles.metaText, styles.metaLink]}>{totalUnchecked} {uncheckedLabel}</ThemedText>
            </Pressable>
          </View>
          ...
          <View style={styles.exportActions}>
            ...
            <Pressable
              style={[styles.exportButton, { backgroundColor: event.archived ? '#0f766e' : '#111827' }]}
          ...
          <TextInput
            style={[styles.searchInput, { color: textColor, backgroundColor: surfaceColor }]}
            value={searchTerm}
                ...
                return (
                  ...
                  >
                    <Pressable
                      ...
                      onPress={() =>
                        preEventStage ? openParticipant(participant.id) : toggleAttended(participant.id)
                      }
                      disabled={isUpdating}
                    >
                      <IconSymbol
                      ...
                      <View style={styles.attendanceNameWrap}>
                        <ThemedText style={styles.attendanceName}>{participant.name}</ThemedText>
                    ...
                    <Pressable
                      ...
                      onPress={() =>
                        preEventStage ? openParticipant(participant.id) : toggleNotAttended(participant.id)
                      }
                      disabled={isUpdating}
                    >
                      <ThemedText
                        style={[
                          ...
                          notAttended ? styles.notAttendedButtonTextActive : null,
                        ]}
                      >
