...
export default function EventDetailScreen() {
  ...
  const handleShareEvent = async () => {
    ...
    const webUrl = `${WEB_BASE_URL}/public-event?eventId=${event.id}`;
  ...
  const handleShareWithParticipants = async () => {
    ...
    const messages = event.participants.map(participant => {
      const webUrl = `${WEB_BASE_URL}/public-event?eventId=${event.id}&name=${encodeURIComponent(participant.name)}`;
