...
// TEMP auth cache for OAuth redirect handoff
export const recentAuth = new Map<string, { token: string; expires: number }>();

export const AUTH_GRACE_MS = 60_000; // 60 seconds
...
let sessionCookie = token || cookies.get(COOKIE_NAME);

// OAuth redirect grace window (frontend has token but no cookie yet)
if (!sessionCookie) {
  const q = req.query as any;
  const graceKey =
    (typeof q?.sessionToken === "string" && q.sessionToken) ||
    (typeof q?.token === "string" && q.token) ||
    null;

  if (graceKey) {
    const cached = recentAuth.get(graceKey);
    if (cached && cached.expires > Date.now()) {
      sessionCookie = cached.token;
      recentAuth.delete(graceKey); // one-time use
    }
  }
}
...
try {
  ...
  return {
    ...
    try {
      ...
      console.warn("[Database] upsertUser failed (continuing without DB)");
    }
