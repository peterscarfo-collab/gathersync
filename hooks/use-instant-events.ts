...
function subscribePendingDeleteIds(listener: () => void) {
  ...
  return () => {
    pendingDeleteListeners.delete(listener);
  };
}
...
function getEventDeleteTransaction(eventId: string) {
  const eventTx = (db as any).tx?.events?.[eventId];
  if (!eventTx?.delete) {
    throw new Error('[useEvents] InstantDB events delete transaction API is unavailable');
  }

  return eventTx.delete();
}
...
export function useEvent(eventId: string | null) {
  ...
  const isPendingDelete = Boolean(eventId && pendingDeletedEventIds.has(eventId));
  
  // Query single event with participants
  const { data, isLoading, error } = db.useQuery(
    user?.id && eventId && !isPendingDelete
      ? {
          events: {
            $: {
              where: {
