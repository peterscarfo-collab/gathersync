...
function getEventDeleteTransaction(eventId: string) {
  const txRoot = (db as any).tx;
  if (!txRoot?.events?.[eventId]) {
    throw new Error('[useEvents] InstantDB events transaction builder is unavailable');
  }
...
async function deleteInstantEvent(eventId: unknown, expectedUserId?: string): Promise<string> {
  ...
  try {
    ...
    const ownerId = event.creator?.id;
    if (expectedUserId && String(ownerId || '') !== String(expectedUserId)) {
      throw new Error(
        `[useEvents] User ${expectedUserId} cannot delete event ${normalizedEventId} owned by ${ownerId || 'unknown'}`
      );
    }
    ...
    await withTimeout(
      (db as any).transact([getEventDeleteTransaction(normalizedEventId)]),
      10000,
...
export function useEvents() {
  ...
  const { user, isLoading: authLoading = false, error: authError } = db.useAuth();
